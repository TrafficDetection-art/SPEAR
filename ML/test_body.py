...
# 3. 加载全量测试数据
data_path = '../email_ccs/outputs/agent-gpt-4o/20250707_213314/iteration_1_emails.json'  # 根据实际情况修改路径
...
print(f"原始数据加载后的大小: {len(test_df)}")

# 检查数据的基本信息
print(f"数据列名: {list(test_df.columns)}")
print(f"Text列的非空值数量: {test_df['Text'].notna().sum()}")
print(f"Class列的非空值数量: {test_df['Class'].notna().sum() if 'Class' in test_df.columns else 'Class列不存在'}")
...
if dropped_text_rows > 0:
    ...
    print(f"删除Text为NaN后剩余数据: {len(test_df)}")

print(f"全量测试数据大小: {len(test_df)}")
...
if 'Class' in test_df.columns:
    ...
    if rows_before > rows_after:
        ...
        print(f"删除Class为NaN后剩余数据: {len(test_df)}")
        # 因为删除了行，所以需要重新准备测试数据
        X_test = test_df['Text']
        X_test_tfidf = vectorizer.transform(X_test)
    ...
    print(f"最终用于评估的数据大小: {len(test_df)}")
    print("模型性能评估（全量数据）：")
    print("-" * 50)
    ...
    for model_name, model in models.items():
        ...
        # 添加调试信息
        print(f"预测结果分布: {np.bincount(y_pred)}")
        print(f"真实标签分布: {np.bincount(y_test)}")
        print(f"预测结果中类别1的数量: {(y_pred == 1).sum()}")
        print(f"真实标签中类别1的数量: {(y_test == 1).sum()}")
